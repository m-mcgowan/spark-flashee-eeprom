...
inline uint8_t* as_bytes(void* buf) { return (uint8_t*)buf; }
inline const uint8_t* as_bytes(const void* buf) { return (const uint8_t*)buf; }
...
public:
     ...
     * @return The size of each page in this flash device.
     */
    virtual page_size_t pageSize() const { return pageSize_; }

    /**
     * @return The number of pages in this flash device.
     */
    virtual page_count_t pageCount() const { return pageCount_; }
...
public:
     ...
     * @return The size of each page in this flash device.
     */
    virtual page_size_t pageSize() const { return flash.pageSize(); }

    /**
     * @return The number of pages in this flash device.
     */
    virtual page_count_t pageCount() const { return flash.pageCount(); }
...
/**
 * Provides reads and writes that can span page boundaries.
 */
...
/**
 * Provides a subrange of an existing flash device. Addresses must be on a page
 * boundary. The start address is inclusive, the end address is exclusive.
 */
...
public:
     ...
     */
    virtual page_size_t pageSize() const { return flash.pageSize()>>SLOT_SIZE_SHIFT; }
...
public:
    ...
    /**
     * The maximum address that can be written. This is one page below the maximum
     * address for the delegate flash device.
     * @return  The maximum address that can be written to (excludive).
     */
    flash_addr_t maxAddress() const { return (flash.pageCount()-1)*flash.pageSize(); }

    bool isValidRegion(flash_addr_t address, page_size_t length) const {
        return address+length <= maxAddress();
    }

    /**
     * @return The number of pages in this flash device.
     */
    virtual page_count_t pageCount() const { return super::pageCount()-1; }
     ...
     */
    virtual bool writeErasePage(const void* data, flash_addr_t address, page_size_t length) {
        // attempt to write and compare. if different, then copy the page to the
        // temporary page, erase original page, copy back and then write the new buffer.
        return false;
    }
...
class CircularBuffer {
    ...
    const flash_addr_t size;

public:

    CircularBuffer(FlashDevice& storage)
        : flash(storage), write_pointer(0), read_pointer(0), size(flash.pageAddress(flash.pageCount()))
    {
    }
    ...
    page_size_t read(void* buf, page_size_t length) const {
        ...
        if (diff>0) {
            ...
            toRead = min(size-read_pointer, flash_addr_t(length));
        }
        if (toRead>0) {
            flash.readPage(buf, read_pointer, toRead);
            read_pointer += toRead;
    ...
    page_size_t write(void* buf, page_size_t length) {
        ...
        if (diff>0) {
            toWrite = min(size-write_pointer, flash_addr_t(length));
        }
        ...
        if (toWrite>0) {
            flash.writePage(buf, write_pointer, toWrite);
            write_pointer += toWrite;
...
class SparkExternalFlashDevice : public FlashDevice {
     ...
     * @return The size of each page in this flash device.
     */
    virtual page_size_t pageSize() const { return sFLASH_PAGESIZE; }

    /**
     * @return The number of pages in this flash device.
     */
    virtual page_count_t pageCount() const { return 512; }
...
public:
    ...
    /**
     * Creates a flash device where destructive writes cause a page erase, but
